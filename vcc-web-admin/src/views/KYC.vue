<template>
  <div class="app-container">
    <div class="filter-container">
      <el-select v-model="listQuery.status" placeholder="审核状态" clearable style="width: 150px;">
    ...
    <el-table :data="list" v-loading="loading" border style="width: 100%; margin-top: 16px;">
      <el-table-column prop="id" label="申请 ID" width="180" />
      <el-table-column prop="email" label="用户邮箱" />
      <el-table-column prop="type" label="认证类型">
        <template #default="scope">
          {{ scope.row.type === 'PERSONAL' ? '个人认证' : '企业认证' }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="scope">
          ...
          <el-button v-if="scope.row.status === 'PENDING'" size="small" type="success" @click="handleApprove(scope.row)">通过</el-button>
...
const list = ref([])
const total = ref(0)
const listQuery = ref({ page: 1, limit: 20, status: '', type: '' })
...
const handleApprove = async (row) => {
  try {
    await ElMessageBox.confirm('确认通过该KYC申请？', '提示', { type: 'warning' })
...
const confirmReject = async () => {
  try {
    ...
    await client.post(`/admin/kyc/${currentRow.value.id}/reject`, { reason: rejectForm.value.reason })
