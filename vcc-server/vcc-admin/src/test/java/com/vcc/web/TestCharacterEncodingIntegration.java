...
/**
 * 中文编码集成测试 (VCC-011) - 字符编码单元测试
 *
 * 测试目标: 验证后端 API 中文编码问题完全解决
 * 测试范围:
 * 1. 验证 API 返回的中文数据不乱码，正确显示
 ...
 * @author VCC-QA
 */
@DisplayName("中文编码集成测试")
class TestCharacterEncodingIntegration {

    private final ObjectMapper objectMapper = new ObjectMapper();

    // ==================== 测试用例 1: 中文字符 UTF-8 编码 ====================
    ...
    void testChineseCharacterUTF8Encoding() {
        ...
        assertThat(bytes.length)
            .as("验证中文字符占用 UTF-8 字节数（4个中文字符应占 12 个字节）")
            .isEqualTo(12);
    ...
    void testEmojiCharacterEncoding() {
        ...
        // 验证至少有 4 字节的 emoji（主要的 emoji 类型）
    ...
    void testUTF8BOM() {
        ...
        System.out.println("  有 UTF-8 BOM: " + hasBom);
        System.out.println("  前三个字节: " + String.format("%02x %02x %02x", 
            bytes[0], bytes[1], bytes[2]));
        
        // 标准 UTF-8 编码不应该包含 BOM
        assertThat(hasBom)
            .as("验证标准 UTF-8 编码不包含 BOM")
            .isFalse();
        
        System.out.println("✓ 测试 5 通过: UTF-8 BOM 验证通过\n");
    }
